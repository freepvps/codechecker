...
def distance_to_probability(v, sigm=1.0):
    v = np.asarray(v)
    return softmax(-v * v / (sigm * sigm))
...
def make_index(sentences):
    ...
    probs = [distance_to_probability(v) for v in dist_matrix]
    return map(np.ndarray.tolist, probs)
